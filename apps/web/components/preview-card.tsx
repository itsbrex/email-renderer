...
const clientIcons: Record<string, React.ReactNode> = {
  ...
  'yahoo-mail': (
    <svg
      ...
      fill="currentColor"
    >
      <path d="M223.8 141.1l-56.7 143.2-56-143.2-96.1 0 105.8 249.1-38.6 89.8 94.2 0 140.9-338.9-93.6 0zM329.2 276.9a58.2 58.2 0 1 0 0 116.4 58.2 58.2 0 1 0 0-116.4zM394.7 32l-93 223.5 104.8 0 92.6-223.5-104.4 0z" />
    </svg>
  ),
};
...
export function PreviewCard({ result, isLoading, clientId }: PreviewCardProps) {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-zinc-700 bg-zinc-800">
      <div className="flex items-center justify-between border-b border-zinc-700 bg-zinc-800 py-2 pr-2 pl-3">
        ...
        <div className="flex items-center gap-2">
          <Badge
            variant="outline"
            className={`px-2 py-0.5 text-xs ${engineStyle.bg} ${engineStyle.text} ${engineStyle.border}`}
          >
            {clientInfo.engine}
          </Badge>
          {clientInfo.simulated && (
            <Badge
              variant="outline"
              className="border-amber-400 bg-amber-500/10 text-xs text-amber-400"
            >
              simulated
            </Badge>
          )}
      ...
      <div className="bg-zinc-900/50">
        {isLoading ? (
          <div className="flex items-center justify-center px-3 py-12 text-white">
            <svg className="mb-2 h-6 w-6 animate-spin" fill="none" viewBox="0 0 24 24">
              <circle
          ...
          <div className="flex h-[200px] flex-col items-center justify-center p-4 text-zinc-500">
            <svg
