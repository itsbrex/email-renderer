...
function WarningItem({ warning }: { warning: Warning }) {
  ...
  return (
    <div className={`mb-2 rounded border p-2.5 ${config.bg} ${config.border}`}>
      <div className={`flex items-start gap-2 ${config.text}`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs text-zinc-300">{warning.message}</p>
...
function ClientInsights({ analysis }: { analysis: AnalysisResult }) {
  ...
  return (
    <AccordionItem value={analysis.clientId} className="border-b border-zinc-700/50">
      <AccordionTrigger className="px-3 py-2 text-sm hover:bg-zinc-800/50 hover:no-underline">
        <div className="flex w-full items-center gap-2">
          <span className="font-medium text-zinc-200">{clientInfo.name}</span>
          <div className="mr-2 ml-auto flex items-center gap-1.5">
            {errorCount > 0 && (
      ...
      <AccordionContent className="px-3 pb-3">
        <div className="space-y-3">
          <div>
            <h4 className="mb-2 text-[10px] font-semibold tracking-wide text-zinc-500 uppercase">
              Compatibility Issues
            </h4>
            {sortedWarnings.length > 0 ? (
              sortedWarnings.map((warning, i) => <WarningItem key={i} warning={warning} />)
            ) : (
